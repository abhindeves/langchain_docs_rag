...
title: API Service
description: Overview of the Search API service, detailing its architecture and hybrid search implementation using Qdrant.
tags: [architecture, api, rag, search, qdrant]
verified:
  - by: openwiki/0.4.0
...
sequenceDiagram
    ...
    User->>API: POST /v1/retrieve (query_text, hybrid=True)
...
The API Service is configurable via the `api-service` environment settings, including:
...
*   **Middleware:** The service includes custom middleware for handling cross-cutting concerns like logging and security.
