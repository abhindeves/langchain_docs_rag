...
The API Service provides the main interface for querying the RAG platform. It is a FastAPI-based application that exposes endpoints for health checks, embedding, retrieval, and chat.
...
*   `POST /v1/retrieve`
    *   Performs hybrid (dense + sparse) semantic search over the document corpus.
*   `POST /v1/chat`
    *   Engages in a chat-based interaction with the RAG model.
...
*   **`v1/`:** Houses the version 1 API endpoints, including retrieval and chat functionalities.

## Future Development

(Currently no major features in development.)
